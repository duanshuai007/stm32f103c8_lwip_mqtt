...
static uint32_t info_hdr_len=0;
static uint32_t info_data_len=0;
...
uint32_t checksum(uint8_t *buf, uint32_t len,uint8_t type)
{
    // type 0=ip 
    //      1=udp
    //      2=tcp
...
void make_arp_answer_from_request(uint8_t *buf)
{
    ...
    //
    make_eth(buf);
    buf[ETH_ARP_OPCODE_H_P]=ETH_ARP_OPCODE_REPLY_H_V;   //arp ?имио|
...
void make_echo_reply_from_request(uint8_t *buf,uint32_t len)
{
    make_eth(buf);
    make_ip(buf);
    buf[ICMP_TYPE_P]=ICMP_TYPE_ECHOREPLY_V;	  //////???икио|бфe////////////////////////////////////////////////////////////////////////////
...
void make_udp_reply_from_request(uint8_t *buf,uint8_t *data,uint32_t datalen,uint32_t port)
{
    uint32_t i=0;
    uint32_t ck;
    make_eth(buf);
